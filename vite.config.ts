  ...
  return {
    ...
    server: {
      ...
      proxy: {
        ...
        // any device that can reach this dev server, same reasoning as the
 ...
 * if the page reloads after Vite already delivered an error to a
 * live socket, the new socket misses the payload and the overlay
 * stays hidden even though the build is still broken. We intercept
 * `ws.send` to remember the latest error and replay it on every new
 * connection; the cache clears on a successful `update` or
